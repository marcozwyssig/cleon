...
import cleon.architecturemethods.arc42.metamodel.spec._09_concepts.system.security.identity.authz.deployment.system.SystemPackage;
	...
	public void validate(final ValidationContext context, final List<IResourceInconsistency> inconsistencyList) {
		...
		try {
			...
			if (name.length() > MAX_LENGTH_AD_GROUP) {
				...
				inconsistencyList.add(new PredicateInconsistency(context.getPackage(), context.getResource(), SystemPackage.ActivityPermission, errormessage));
			}
		} finally {
