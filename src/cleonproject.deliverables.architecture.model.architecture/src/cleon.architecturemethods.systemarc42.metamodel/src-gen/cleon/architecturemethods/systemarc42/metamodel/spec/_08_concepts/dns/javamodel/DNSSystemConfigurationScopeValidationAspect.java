...
import cleon.architecturemethods.systemarc42.metamodel.spec._08_concepts.dns.FunctionSpace_DNS.IDNSSystemConfigurationScopeFunctions;

public class DNSSystemConfigurationScopeValidationAspect implements IResourceValidationAspect {
	...
	public void validate(final ValidationContext context, final List<IResourceInconsistency> inconsistencyList) {
		...
		final var dnsSystemConfigurationScope = resourceRepository.getResource(IDNSSystemConfigurationScope.class, context.getResource());
		final var dnsSystemConfigurationScopeFunctions = dnsSystemConfigurationScope.extension(IDNSSystemConfigurationScopeFunctions.class);
		...
		if (dnsSystemConfigurationScopeFunctions.HasAccessConfigurationTo() && !dnsSystemConfigurationScope.selectAllowDNSRecords() ) {
			inconsistencyList.add(new PredicateInconsistency(context.getPackage(), context.getResource(),
					DnsPackage.DNSSystemConfigurationScope_allowDNSRecords, "allowed DNS records needs to be enabled (used in access)"));
		}
